...
# Output ends up in /app/seanime-web/out
...
COPY --from=web-builder /app/seanime-web/out ./web

# CGO is required (mattn/go-sqlite3, etc.). Builds a dynamically-linked binary.
RUN CGO_ENABLED=1 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/seanime ./
...
RUN apt-get update \
 ...
 && rm -rf /var/lib/apt/lists/* \
 && useradd -r -m -d /home/kuro -s /sbin/nologin kuro

WORKDIR /app
COPY --from=go-builder /out/seanime /app/seanime
