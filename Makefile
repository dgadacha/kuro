# kuro — one-command dev + build helpers.
# Usage:
...
install-deps:
	@if [ ! -d seanime-web/node_modules ]; then \
	  echo "→ installing npm deps (one-off)..."; \
	  cd seanime-web && $(NPM) install; \
	fi
...
build: ## Build the standalone binary (web embedded)
	...
	@CGO_ENABLED=1 $(GO) build -o seanime -trimpath -ldflags="-s -w"
	@echo "→ done: ./seanime"

run: build ## Build then launch the prod binary
	@./seanime --datadir="$(DATADIR)"

clean: ## Remove build artifacts (web/, seanime, seanime-web/out)
	@rm -rf web seanime seanime-web/out
